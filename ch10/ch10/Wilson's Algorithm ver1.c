...
//Wilson's Algorithm
//
//하나의 셀(도착점)을 정해 놓고, 다른 곳(출발점)에서 좌우상하 임의의 방향으로 전진하기를 반복하다가 도착점을 만나면 지금까지의 경로에 대하여 미로 표시한다.
//또 다른 곳에서 출발하여 미로를 만날 때까지 좌우상하 임의의 방향으로 전진하다가 미로를 만나면 지금까지의 경로를 미로 표시하고, 모든 셀이 미로의 경로를 가질 때가지 이 과정을 반복한다.
//단순한 방법으로 최초로 정해 놓은 셀, 그리고 미로를 만나기 위해 헤매는 반복횟수가 많은 방법임
//*/
//
//#include<stdio.h>
//#include<stdlib.h>
